...
from getdata import getData, preprocess_input
...
# 载入数据集
datafile = 'fer2013/fer2013.csv'
faces, emotions = getData(datafile)
...
# 利用训练好的模型对测试集进行预测
ndata = X_test.shape[0]  # 测试集数据量
y_pred = np.zeros((ndata,))  # 用于保存预测的结果
y_true = [y_test[i].argmax() for i in range(ndata)]  # 获取真实标签
